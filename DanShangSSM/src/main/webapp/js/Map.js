...
window.onload = function () {
    ...
    BigZoomControl.prototype.initialize = function (map) {
       ...
       /* div.style.margin = "0 auto";*/
        div.innerHTML = "<div style='margin-right:-100px'><input type='text' id = 'baiduAddress' name = 'baiduAddress' style='width: 200px;height: 33px;text-indent:1em;float:left;border: 1px solid #D2D2D2!important;border-right:none;' placeholder='请输入地址！' /><div  class='searchMap' style='border:1px solid #f50;background-color:#f50;width:60px;height:33px;float:left;'></div><img  class='searchMap' src='img/search.png' style='height: 22px;position: absolute;right: -80px;top: 7px;'/><div style='clear:both'></div></div> ";
        // 添加DOM元素到地图中
...
//创建导航坐标
   function BigZoomControl1() {
        // 设置默认停靠位置和偏移量
        this.defaultAnchor = BMAP_ANCHOR_TOP_RIGHT;
        this.defaultOffset = new BMap.Size(10, 20);
    }
    // 通过JavaScript的prototype属性继承于BMap.Control
   BigZoomControl1.prototype = new BMap.Control();
   BigZoomControl1.prototype.initialize = function (map) {
        // 创建一个DOM元素
        var screenHeight1 = window.screen.height;
        var div1 = document.createElement("div");
        // 设置样式
        div1.style.cursor = "pointer";
         div1.style.position="fixed";
         div1.style.marginTop=(100)+"px";
       /* d1iv.style.margin = "0 auto";*/
        div1.innerHTML = "<button onclick='toudi()' class='order'  style='margin-left:10px;border: 0;display: block;  background: #1c212e; position: relative;height: 45px;width:45px; padding: 0; outline: none; border-radius: 32px; overflow: hidden;'><span class='default' style='  position: absolute;left: 0;    right: 0;text-align: center;top: 19px;    line-height: 11px; color: white;font-size: 16px;font-weight: 500; '>投递</span></button><button  onclick='shangmenfuwu()' class='order'  style='display: block; margin-top:10px;   margin-left:10px;border: 0; background: #1c212e; position: relative;     height: 45px;     width: 45px; padding: 0; outline: none; border-radius: 32px; overflow: hidden;'><span class='default' style='position: absolute;left: 0;right: 0;text-align: center;top: 19px;line-height: 11px; color: white;font-size: 16px;font-weight: 500; '>上门</span></button> <images src=''/>  ";
        // 添加DOM元素到地图中
    ...
    var myZoomCtrl = new BigZoomControl();
     // 创建控件2
    var myZoomCtrl1 = new BigZoomControl1();
    //// 查询控件类到地图当中
